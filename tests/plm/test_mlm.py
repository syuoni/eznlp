...
class TestMaskedLM(object):
    ...
    def test_ResumeNER(self, ResumeNER_demo, device):
        ...
        io = RawTextIO(
            ...
            jieba.tokenize,
            max_len=128,
            document_sep_starts=["-DOCSTART-", "<doc", "</doc"],
            encoding="utf-8",
        )
        ...
        self.batch = self.dataset.collate([self.dataset[i] for i in range(4)]).to(
            self.device
        )
