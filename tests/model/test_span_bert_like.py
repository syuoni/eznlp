...
def test_span_bert_like(min_span_size, use_init_size_emb, bert_like_with_tokenizer):
    ...
    assert all(
        all_last_query_states[k].size(1) == 10 - k + 1 for k in range(min_span_size, 6)
    )
    ...
    assert (diff > 1.0).sum().item() == diff.size(0) * (
        diff.size(0) - 1
    )  # 40*39 = 1560
...
def test_trainble_config(
    min_span_size,
    use_init_size_emb,
    share_weights_int,
    freeze,
    bert_like_with_tokenizer,
):
    bert_like, tokenizer = bert_like_with_tokenizer
    config = SpanBertLikeConfig(
        bert_like=bert_like,
