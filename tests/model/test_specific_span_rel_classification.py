...
class TestModel(object):
    ...
    @pytest.mark.parametrize(
        "use_unfiltered, num_layers, min_span_size, use_context, context_mode, fusing_mode, ck_loss_weight",
        [
    ...
    def test_model(
        ...
        if use_unfiltered:
            decoder_config = UnfilteredSpecificSpanRelClsDecoderConfig(
                fusing_mode=fusing_mode, min_span_size=min_span_size, max_span_size=3
            )
        else:
    ...
    def test_prediction_without_gold(
        self, use_unfiltered, conll2004_demo, bert_with_tokenizer, device
    ):
        bert, tokenizer = bert_with_tokenizer
        self.config = SpecificSpanExtractorConfig(
            decoder="unfiltered_specific_span_rel"
            if use_unfiltered
            else "specific_span_rel",
            bert_like=BertLikeConfig(
                tokenizer=tokenizer,
                bert_like=bert,
                freeze=False,
