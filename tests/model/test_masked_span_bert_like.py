...
def test_masked_span_bert_like(
    min_span_size, use_init_size_emb, bert_like_with_tokenizer
):
    bert_like, tokenizer = bert_like_with_tokenizer
    bert_like.eval()
    spb_config = SpanBertLikeConfig(
        bert_like=bert_like,
    ...
    if min_span_size == 1:
        ...
        all_hidden = [bert_outs["last_hidden_state"]] + list(
            all_last_query_states.values()
        )

    batch_chunks = [
        [("EntA", 0, 1), ("EntB", 1, 2), ("EntA", 2, 3)],
        [("EntA", 0, 20)],
    ...
    batch_span_size_ids = [
        torch.tensor([end - start - 1 for label, start, end in chunks])
        for chunks in batch_chunks
    ]
    batch_span_size_ids = torch.nn.utils.rnn.pad_sequence(
        batch_span_size_ids, batch_first=True, padding_value=0
    )
    ...
    mspb_config = MaskedSpanBertLikeConfig(
        bert_like=bert_like, use_init_size_emb=use_init_size_emb
    )
    mspb_config.max_size_id = 3
    masked_span_bert_like = mspb_config.instantiate()
    if use_init_size_emb:
        masked_span_bert_like.size_embedding = copy.deepcopy(
            span_bert_like.size_embedding
        )
    masked_span_bert_like.eval()
    all_last_query_states = masked_span_bert_like(
        bert_outs["hidden_states"],
        span_size_ids=batch_span_size_ids,
        cp_dist_ids=None,
        ck2tok_mask=batch_ck2tok_mask,
        ctx2tok_mask=batch_ctx2tok_mask,
    )
    ...
    for k, chunks in enumerate(batch_chunks):
        for i, (_, start, end) in enumerate(chunks):
            ...
            if (min_span_size == 1 or end - start > 1) and end - start <= 5:
                delta_hidden = (
                    span_query_hidden[k, i] - all_hidden[end - start - 1][k, start]
                )
...
def test_trainble_config(use_init_size_emb, freeze, bert_like_with_tokenizer):
    bert_like, tokenizer = bert_like_with_tokenizer
    config = MaskedSpanBertLikeConfig(
        bert_like=bert_like, use_init_size_emb=use_init_size_emb, freeze=freeze
    )
