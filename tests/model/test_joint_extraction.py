...
class TestModel(object):
    def _assert_batch_consistency(self):
        ...
        assert all(
            tuples_pred012[1:] == tuples_pred123[:-1]
            for tuples_pred012, tuples_pred123 in zip(y_pred012, y_pred123)
        )
    ...
    def test_prediction_without_gold(self, HwaMei_demo, device):
        self.config = ExtractorConfig(
            decoder=JointExtractionDecoderConfig(attr_decoder="span_attr")
        )
        self._setup_case(HwaMei_demo, device)
        ...
        assert all(
            len(set_tuples_pred) == len(data_wo_gold)
            for set_tuples_pred in trainer.predict(dataset_wo_gold)
        )

    @pytest.mark.parametrize("use_unfiltered", [False, True])
    def test_model_with_specific_span(
        self, use_unfiltered, conll2004_demo, bert_with_tokenizer, device
    ):
        bert, tokenizer = bert_with_tokenizer
        if use_unfiltered:
            rel_decoder_config = UnfilteredSpecificSpanRelClsDecoderConfig(
                max_span_size=3
            )
        else:
        ...
        self.config = SpecificSpanExtractorConfig(
            decoder=JointExtractionDecoderConfig(
                ck_decoder=SpecificSpanClsDecoderConfig(max_span_size=3),
    ...
    @pytest.mark.parametrize("use_unfiltered", [False, True])
    def test_prediction_without_gold_for_specific_span(
        self, use_unfiltered, conll2004_demo, bert_with_tokenizer, device
    ):
        bert, tokenizer = bert_with_tokenizer
        if use_unfiltered:
