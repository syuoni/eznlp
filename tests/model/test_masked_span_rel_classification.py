...
class TestModel(object):
    ...
    @pytest.mark.parametrize(
        "num_layers, use_init_size_emb, use_init_dist_emb, use_context, context_mode, context_ext_win, context_exc_ck, fusing_mode, red_dim, ss_epsilon, ck_loss_weight, use_inv_rel",
        [
            (
                3,
                False,
            ...
            (
                ...
                0.0,
                False,
            ),  # Use initial size/dist embedding
            (
                3,
                False,
                True,
    ...
    def test_model(
        ...
        decoder_config = MaskedSpanRelClsDecoderConfig(
            use_context=use_context,
            context_mode=context_mode,
            context_ext_win=context_ext_win,
            context_exc_ck=context_exc_ck,
        ...
        self.config = MaskedSpanExtractorConfig(
            decoder=decoder_config,
            bert_like=BertLikeConfig(
                tokenizer=tokenizer,
                bert_like=bert,
                freeze=False,
                from_subtokenized=True,
                output_hidden_states=True,
            ),
            masked_span_bert_like=MaskedSpanBertLikeConfig(
                bert_like=bert,
                freeze=False,
                num_layers=num_layers,
                use_init_size_emb=use_init_size_emb,
                use_init_dist_emb=use_init_dist_emb,
...
def test_consistency_with_span_bert_like(
    min_span_size, use_init_size_emb, conll2004_demo, bert_with_tokenizer, device
):
    bert, tokenizer = bert_with_tokenizer

    sse_config = SpecificSpanExtractorConfig(
        decoder=SpecificSpanClsDecoderConfig(
            min_span_size=min_span_size, max_span_size=5
        ),
        bert_like=BertLikeConfig(
            tokenizer=tokenizer,
            bert_like=bert,
        ...
        span_bert_like=SpanBertLikeConfig(
            bert_like=bert, use_init_size_emb=use_init_size_emb
        ),
        intermediate2=None,
    )
    preprocessor = BertLikePreProcessor(tokenizer, verbose=False)
    ...
    mse_config = MaskedSpanExtractorConfig(
        decoder="masked_span_rel",
        bert_like=BertLikeConfig(
            tokenizer=tokenizer,
            bert_like=bert,
    ...
    if use_init_size_emb:
        mse_model.masked_span_bert_like.size_embedding = copy.deepcopy(
            sse_model.span_bert_like.size_embedding
        )
    mse_model.eval()
    ...
    for k, entry in enumerate(conll2004_demo[:8]):
        for i, (label, start, end) in enumerate(entry["chunks"]):
            ...
            if (min_span_size == 1 or end - start > 1) and end - start <= 5:
                delta_hidden = (
                    mse_states["span_query_hidden"][k, i]
                    - sse_states["all_query_hidden"][end - start][k, start]
                )
