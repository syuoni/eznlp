...
class TestModel(object):
    ...
    @pytest.mark.parametrize(
        "arch", ["FFN", "Conv", "Gehring", "LSTM", "GRU", "Transformer"]
    )
    @pytest.mark.parametrize("shortcut", [False, True])
    @pytest.mark.parametrize(
        "use_crf, fl_gamma, sl_epsilon",
        [(True, 0.0, 0.0), (False, 2.0, 0.0), (False, 0.0, 0.1)],
    )
    def test_model(
        self, arch, shortcut, use_crf, fl_gamma, sl_epsilon, conll2003_demo, device
    ):
        self.config = ExtractorConfig(
            intermediate2=EncoderConfig(
                arch=arch, shortcut=shortcut, use_emb2init_hid=True
            ),
            decoder=SequenceTaggingDecoderConfig(
                use_crf=use_crf, fl_gamma=fl_gamma, sl_epsilon=sl_epsilon
            ),
        )
    ...
    def test_model_with_char(self, arch, conll2003_demo, device):
        ...
        self.config = ExtractorConfig(
            "sequence_tagging",
            nested_ohots=ConfigDict({"char": char_config}),
        )
        self._setup_case(conll2003_demo, device)
        self._assert_batch_consistency()
        self._assert_trainable()
    ...
    def test_model_with_softlexicon(self, ctb50, ResumeNER_demo, device):
        ...
        self.config = ExtractorConfig(
            "sequence_tagging",
            nested_ohots=ConfigDict({"softlexicon": SoftLexiconConfig(vectors=ctb50)}),
        )
    ...
    def test_model_with_more_fields(self, conll2003_demo, device):
        self.config = ExtractorConfig(
            ...
            mhots=ConfigDict(
                {
                    f: MultiHotConfig(field=f, emb_dim=20)
                    for f in Token._basic_mhot_fields
                }
            ),
        )
        self._setup_case(conll2003_demo, device)
    ...
    def test_model_with_alternative_schemes(self, scheme, conll2003_demo, device):
        self.config = ExtractorConfig(
            decoder=SequenceTaggingDecoderConfig(scheme=scheme)
        )
        self._setup_case(conll2003_demo, device)
        self._assert_batch_consistency()
        self._assert_trainable()
    ...
    def test_model_with_pretrained_vector(
        self, freeze, glove100, conll2003_demo, device
    ):
        self.config = ExtractorConfig(
            "sequence_tagging",
            ohots=ConfigDict(
                {"text": OneHotConfig(field="text", vectors=glove100, freeze=freeze)}
            ),
        )
        self._setup_case(conll2003_demo, device)
        self._assert_batch_consistency()
    ...
    def test_model_with_elmo(self, freeze, elmo, conll2003_demo, device):
        self.config = ExtractorConfig(
            "sequence_tagging", ohots=None, elmo=ELMoConfig(elmo=elmo)
        )
        self._setup_case(conll2003_demo, device)
        self._assert_batch_consistency()
        self._assert_trainable()
    ...
    def test_model_with_bert_like(
        self, freeze, bert_with_tokenizer, conll2003_demo, device
    ):
        ...
        self.config = ExtractorConfig(
            ...
            bert_like=BertLikeConfig(
                bert_like=bert, tokenizer=tokenizer, freeze=freeze
            ),
        )
        self._setup_case(conll2003_demo, device)
        self._assert_batch_consistency()
    ...
    @pytest.mark.parametrize("freeze", [False, True])
    def test_model_with_flair(
        self, freeze, flair_fw_lm, flair_bw_lm, conll2003_demo, device
    ):
        self.config = ExtractorConfig(
            "sequence_tagging",
            ohots=None,
