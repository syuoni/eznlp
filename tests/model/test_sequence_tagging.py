...
class TestModel(object):
    ...
    def test_train_steps(self, use_amp, conll2003_demo, device):
        ...
        dataloader = torch.utils.data.DataLoader(self.dataset, batch_size=4, shuffle=True, collate_fn=self.dataset.collate)
    ...
    @pytest.mark.parametrize("use_amp", [False, True])
    def test_gradient_accumulation(self, use_amp, conll2003_demo, device):
        if use_amp and device.type.startswith('cpu'):
            pytest.skip("test requires cuda, while current session runs on cpu")
        ...
        param_backup = [p1.data.clone() for p1 in model1.parameters()]
        
        
        optimizer1 = torch.optim.AdamW(model1.parameters())
        trainer1 = Trainer(model1, optimizer=optimizer1, use_amp=use_amp, device=device)
        ...
        trainer2 = Trainer(model2, optimizer=optimizer2, num_grad_acc_steps=2, use_amp=use_amp, device=device)
        ...
        assert all((p1 - p2).abs().max().item() < 1e-4 for p1, p2 in zip(model1.parameters(), model2.parameters()))
        assert all((p1 - pb).abs().max().item() > 1e-4 for p1, pb in zip(model1.parameters(), param_backup))
