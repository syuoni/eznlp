...
class TestModel(object):
    ...
    def test_model(
        ...
        self.config = Image2TextConfig(
            encoder=ImageEncoderConfig(
                arch=enc_arch, backbone=backbone, transforms=trans
            ),
    ...
    def test_prediction_without_gold(self, flickr8k_demo, resnet18_with_trans, device):
        ...
        self.config = Image2TextConfig(
            encoder=ImageEncoderConfig(backbone=resnet, transforms=trans)
        )
        self._setup_case(flickr8k_demo, device)
...
def test_generation_dataset(training, flickr8k_demo, resnet18_with_trans):
    ...
    if training:
        ...
        assert all(
            (examples[i]["img"] - examples[0]["img"]).abs().max().item() < 1e-6
            for i in range(5)
        )
        assert all(
            (examples[i]["img"] - examples[0]["img"]).abs().max().item() > 1e-4
            for i in range(5, 10)
        )
...
def test_image_cache(use_cache, flickr8k_demo, resnet18_with_trans, device):
    ...
    config = Image2TextConfig(
        encoder=ImageEncoderConfig(
            backbone=resnet, transforms=trans, use_cache=use_cache
        )
    )
    ...
    dataset.build_vocabs_and_dims()
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=4, collate_fn=dataset.collate
    )
