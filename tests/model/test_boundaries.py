...
from eznlp.model.decoder.boundaries import (
    _diagonal2span,
    _span2diagonal,
...
def test_boundaries_obj(sb_epsilon, sl_epsilon, EAR_data_demo):
    ...
    config = BoundarySelectionDecoderConfig(
        sb_epsilon=sb_epsilon, sl_epsilon=sl_epsilon
    )
    ...
    if sb_epsilon == 0 and sl_epsilon == 0:
        assert all(
            boundaries_obj.label_ids[start, end - 1] == config.label2idx[label]
            for label, start, end in chunks
        )
        labels_retr = [
            config.idx2label[i]
            for i in boundaries_obj.label_ids[
                torch.arange(num_tokens) >= torch.arange(num_tokens).unsqueeze(-1)
            ].tolist()
        ]
    else:
        assert all(
            boundaries_obj.label_ids[start, end - 1].argmax() == config.label2idx[label]
            for label, start, end in chunks
        )
        labels_retr = [
            config.idx2label[i]
        ...
        if sb_epsilon == 0:
            assert (
                boundaries_obj.label_ids[:, :, config.none_idx] < 1
            ).sum().item() == num_chunks
        else:
            assert (
                boundaries_obj.label_ids[:, :, config.none_idx] < 1
    ...
    chunks_retr = [
        ...
        for label, (start, end) in zip(
            labels_retr, _spans_from_upper_triangular(num_tokens)
        )
    ...
    assert decoder._span_size_ids.sum().item() == sum(
        min(k, config.max_size_id) * (num_tokens - k) for k in range(num_tokens)
    )
...
def test_boundaries_obj_for_boundary_smoothing(sb_epsilon, sb_size):
    ...
    assert (boundaries_obj.label_ids.sum(dim=-1) - 1).abs().max().item() < 1e-6
    assert (
        boundaries_obj.label_ids[:, :, 1:].sum() - num_chunks
    ).abs().max().item() < 1e-6
    assert (
        boundaries_obj.label_ids[span_sizes <= 0] - torch.tensor([1.0, 0.0, 0.0])
    ).abs().max().item() < 1e-6

    if sb_size == 1:
        assert (
            boundaries_obj.label_ids[0, 0]
            - torch.tensor([(1 / 4) * sb_epsilon, 1 - (1 / 4) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[0, 3]
            - torch.tensor(
                [(1 / 2) * sb_epsilon, 1 - (3 / 4) * sb_epsilon, (1 / 4) * sb_epsilon]
            )
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[0, 4]
            - torch.tensor(
                [(1 / 2) * sb_epsilon, (1 / 4) * sb_epsilon, 1 - (3 / 4) * sb_epsilon]
            )
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[3, 4]
            - torch.tensor([(3 / 4) * sb_epsilon, 1 - (3 / 4) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[4, 4]
            - torch.tensor([(1 / 4) * sb_epsilon, 1 - (1 / 4) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
    elif sb_size == 2:
        assert (
        ...
        assert (
            boundaries_obj.label_ids[0, 4]
            - torch.tensor(
                [(1 / 2) * sb_epsilon, (1 / 8) * sb_epsilon, 1 - (5 / 8) * sb_epsilon]
            )
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[3, 4]
            - torch.tensor([(5 / 8) * sb_epsilon, 1 - (5 / 8) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[4, 4]
            - torch.tensor([(3 / 8) * sb_epsilon, 1 - (3 / 8) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
    elif sb_size == 3:
        assert (
            boundaries_obj.label_ids[0, 0]
            - torch.tensor([(7 / 36) * sb_epsilon, 1 - (7 / 36) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[0, 3]
            - torch.tensor(
        ...
        assert (
            boundaries_obj.label_ids[0, 4]
            - torch.tensor(
                [(4 / 9) * sb_epsilon, (1 / 9) * sb_epsilon, 1 - (5 / 9) * sb_epsilon]
            )
        ).abs().max().item() < 1e-6
        assert (
            boundaries_obj.label_ids[3, 4]
        ...
        assert (
            boundaries_obj.label_ids[4, 4]
            - torch.tensor([(1 / 3) * sb_epsilon, 1 - (1 / 3) * sb_epsilon, 0.0])
        ).abs().max().item() < 1e-6
...
def test_boundaries_obj_for_neg_sampling(
    neg_sampling_rate, neg_sampling_surr_rate, nested_sampling_rate, training
):
    entry = {
        "tokens": list("abcdefhijk"),
        "chunks": [
            ("EntA", 0, 1),
            ("EntA", 0, 4),
            ("EntB", 0, 5),
    ...
    config = BoundarySelectionDecoderConfig(
        neg_sampling_rate=neg_sampling_rate,
    ...
    if (not training) or (neg_sampling_rate == 1 and nested_sampling_rate == 1):
        ...
        num_non_mask = (
            5
            + 10
            * (neg_sampling_rate + (1 - neg_sampling_rate) * neg_sampling_surr_rate)
        ...
        if all(
            r in (0, 1)
            for r in [neg_sampling_rate, neg_sampling_surr_rate, nested_sampling_rate]
        ):
...
def test_diag_boundaries_pair_obj(training, EAR_data_demo):
    ...
    assert dbp_obj.dbp2label_id.sum().item() == sum(
        config.label2idx[label] for label, *_ in relations
    )
    assert all(
        dbp_obj.dbp2label_id[
            _span2diagonal(h_start, h_end, num_tokens),
            _span2diagonal(t_start, t_end, num_tokens),
        ]
    ...
    for label, ((h_start, h_end), (t_start, t_end)) in zip(
        labels_retr, _span_pairs_from_diagonals(num_tokens, 3)
    ):
        head = (
            dbp_obj.span2ck_label.get((h_start, h_end), config.ck_none_label),
            h_start,
            h_end,
        )
        tail = (
            dbp_obj.span2ck_label.get((t_start, t_end), config.ck_none_label),
            t_start,
            t_end,
        )
        if (
            label != config.none_label
            and head[0] != config.ck_none_label
            and tail[0] != config.ck_none_label
        ):
...
def test_spans_from_upper_triangular(seq_len):
    assert (
        len(list(_spans_from_upper_triangular(seq_len))) == (seq_len + 1) * seq_len // 2
    )


@pytest.mark.parametrize(
    "seq_len, max_span_size",
    [(5, 1), (5, 5), (10, 1), (10, 5), (10, 10), (100, 1), (100, 10), (100, 100)],
)
def test_spans_from_diagonals(seq_len, max_span_size):
    assert (
        len(list(_spans_from_diagonals(seq_len, max_span_size)))
        == (seq_len * 2 - max_span_size + 1) * max_span_size // 2
    )


@pytest.mark.parametrize("seq_len", [5, 10, 100])
def test_spans_from_functions(seq_len):
    assert set(_spans_from_diagonals(seq_len)) == set(
        _spans_from_upper_triangular(seq_len)
    )
...
def test_span2diagonal(seq_len):
    ...
    assert [
        _span2diagonal(start, end, seq_len)
        for start, end in _spans_from_diagonals(seq_len)
    ] == list(range(num_spans))
    assert [_diagonal2span(k, seq_len) for k in range(num_spans)] == list(
        _spans_from_diagonals(seq_len)
    )
