...
class TestModel(object):
    ...
    def test_model(self, arch, num_heads, shortcut, multi30k_demo, device):
        self.config = Text2TextConfig(
            encoder=EncoderConfig(arch=arch, use_emb2init_hid=True),
            decoder=GeneratorConfig(
                arch=arch, use_emb2init_hid=True, num_heads=num_heads, shortcut=shortcut
            ),
        )
    ...
    def test_model_with_rnn(self, init_ctx_mode, multi30k_demo, device):
        self.config = Text2TextConfig(
            decoder=GeneratorConfig(arch="LSTM", init_ctx_mode=init_ctx_mode)
        )
        self._setup_case(multi30k_demo, device)
        self._assert_batch_consistency()
        self._assert_trainable()
    ...
    def test_model_with_transformer(
        self, use_emb2init_hid, sin_positional_emb, weight_tying, multi30k_demo, device
    ):
        self.config = Text2TextConfig(
            embedder=OneHotConfig(
                tokens_key="tokens",
                field="text",
                emb_dim=128,
                has_positional_emb=True,
                sin_positional_emb=sin_positional_emb,
            ),
            encoder=EncoderConfig(
                arch="Transformer", use_emb2init_hid=use_emb2init_hid, hid_dim=128
            ),
            decoder=GeneratorConfig(
                ...
                arch="Transformer",
                use_emb2init_hid=use_emb2init_hid,
                hid_dim=128,
    ...
    def test_model_with_label_smoothing(self, sl_epsilon, multi30k_demo, device):
        self.config = Text2TextConfig(
            decoder=GeneratorConfig(arch="LSTM", sl_epsilon=sl_epsilon)
        )
        self._setup_case(multi30k_demo, device)
        self._assert_batch_consistency()
        self._assert_trainable()
