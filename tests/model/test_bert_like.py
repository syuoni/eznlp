...
from eznlp.model import BertLikeConfig, BertLikePostProcessor, BertLikePreProcessor
...
def test_paired_inputs_config(paired_inputs, bert_like_with_tokenizer):
    bert_like, tokenizer = bert_like_with_tokenizer
    bert_like_config = BertLikeConfig(
        bert_like=bert_like, tokenizer=tokenizer, paired_inputs=paired_inputs
    )

    tokens = TokenSequence.from_tokenized_text(
        [c for c in random.choices(string.ascii_letters, k=100)]
    )
    entry = {
        "tokens": tokens
        + TokenSequence.from_tokenized_text([tokenizer.sep_token])
        + tokens
    }
...
class TestBertLikePreProcessor(object):
    def test_truecase_for_data(self, bert_tokenizer):
        ...
        assert new_data[0]["tokens"].raw_text == [
            "Full",
            "fees",
            "1.875",
            "Reoffer",
    ...
    def test_truncate_for_data(self, mode, bert_tokenizer):
        ...
        data = [
            ...
            if len(data_entry["tokens"]) >= max_len - 10
        ]
        ...
        sub_lens = [
            sum(
                len(word)
                for word in _tokenized2nested(
                    data_entry["tokens"].raw_text, bert_tokenizer
                )
            )
            for data_entry in data
        ]
    ...
    def test_segment_sentences_for_data(self, bert_tokenizer, token_len, max_len):
        preprocessor = BertLikePreProcessor(
            bert_tokenizer, model_max_length=max_len + 2, verbose=False
        )

        tokens = TokenSequence.from_tokenized_text(
            [c * token_len for c in random.choices(string.ascii_letters, k=100)]
        )
        ...
        assert all(
            0 <= start and end <= len(entry["tokens"])
        ...
        chunk_texts_retr = [
            entry["tokens"][start:end].raw_text
            for entry in new_data
            for label, start, end in entry["chunks"]
        ]
        assert chunk_texts_retr == chunk_texts

        span_starts = [0] + numpy.cumsum(
            [len(entry["tokens"]) for entry in new_data]
        ).tolist()
        chunks_retr = [
            (label, span_start + start, span_start + end)
            for entry, span_start in zip(new_data, span_starts)
            for label, start, end in entry["chunks"]
        ]
    ...
    def test_subtokenize_for_data(self, bert_tokenizer):
        ...
        for entry, new_entry in zip(data, new_data):
            for ck, new_ck in zip(entry["chunks"], new_entry["chunks"]):
                assert (
                    "".join(entry["tokens"].raw_text[ck[1] : ck[2]]).lower()
                    == "".join(new_entry["tokens"].raw_text[new_ck[1] : new_ck[2]])
                    .replace(sub_prefix, "")
                    .lower()
                )
    ...
    def test_merge_enchars_for_data(self):
        tokenizer = transformers.BertTokenizer.from_pretrained(
            "assets/transformers/bert-base-chinese", do_lower_case=True
        )
...
class TestBertLikePostProcessor(object):
    ...
    def test_conll2003(self, model_max_length, bert_like_tokenizer, conll2003_demo):
        preprocessor = BertLikePreProcessor(
            bert_like_tokenizer, model_max_length=model_max_length, verbose=False
        )
    ...
    def test_conll2004(self, bert_like_tokenizer, conll2004_demo):
        preprocessor = BertLikePreProcessor(
            bert_like_tokenizer, model_max_length=102, verbose=False
        )
    ...
    def test_HwaMei(self, HwaMei_demo):
        ...
        preprocessor = BertLikePreProcessor(
            tokenizer, model_max_length=202, verbose=False
        )
