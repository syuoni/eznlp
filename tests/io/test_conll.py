...
class TestConllIO(object):
    ...
    def _assert_flatten_consistency(self, data):
        ...
        for entry, f_entry in zip(data, flattened_data):
            assert "".join(f_entry["tokens"].raw_text) == "".join(
                entry["tokens"].raw_text
            )

            chunk_texts = [
                "".join(entry["tokens"].raw_text[s:e]) for _, s, e in entry["chunks"]
            ]
            f_chunk_texts = [
                "".join(f_entry["tokens"].raw_text[s:e])
                for _, s, e in f_entry["chunks"]
            ]
            ...
            if hasattr(entry["tokens"][0], "pos_tag"):
                ...
                assert [
                    f_entry["tokens"].pos_tag[i] for i in cum_char_seq_lens[:-1]
                ] == entry["tokens"].pos_tag

    @pytest.mark.parametrize(
        "doc_level, merge_mode",
        [[False, None], [True, "greedy"], [True, "min_max_length"]],
    )
    def test_conll2003(self, doc_level, merge_mode, bert_tokenizer):
        self.io = ConllIO(
            text_col_id=0,
            tag_col_id=3,
            scheme="BIO1",
            document_sep_starts=["-DOCSTART-"],
            additional_col_id2name={1: "pos_tag"},
        )
        ...
        if not doc_level:
            ...
            train_data = preprocessor.merge_sentences_for_data(
                train_data, doc_key="doc_idx", mode=merge_mode
            )
            dev_data = preprocessor.merge_sentences_for_data(
                dev_data, doc_key="doc_idx", mode=merge_mode
            )
            test_data = preprocessor.merge_sentences_for_data(
                test_data, doc_key="doc_idx", mode=merge_mode
            )
    ...
    def test_ontonotesv5(self, doc_level, merge_mode, bert_tokenizer):
        self.io = ConllIO(
            text_col_id=3,
            tag_col_id=10,
            scheme="OntoNotes",
    ...
    def test_clerd(self):
        self.io = ConllIO(
            text_col_id=0, tag_col_id=1, scheme="BIO2", tag_sep="_", encoding="utf-8"
        )
        ...
        data = post_io.map(
            data,
            chunk_type_mapping=lambda x: x.title()
