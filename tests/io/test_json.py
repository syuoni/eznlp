...
class TestJsonIO(object):
    ...
    def test_ace2004(self):
        ...
        assert (
            sum(count_nested(ex["chunks"]) for ex in train_data + dev_data + test_data)
            == 7_832
        )
        assert (
            max(
    ...
    def test_genia(self):
        ...
        assert (
            sum(
                len(set([(s, e) for _, s, e in ex["chunks"]]))
                for ex in train_data + dev_data + test_data
            )
            == 56_015
        )
        ...
        assert (
            len(set([ex["doc_key"] for ex in train_data + dev_data + test_data]))
            == 2_000
        )
    ...
    def test_kbp2017_shen2022acl(self):
        ...
        assert (
            sum(len(ex["chunks"]) for ex in train_data) == 31_235
        )  # 31_236 reported in Shen et al. (2022), due to a duplicated entity
        assert len(dev_data) == 545
        assert sum(len(ex["chunks"]) for ex in dev_data) == 1_879
        assert len(test_data) == 4_267
        assert (
            sum(len(ex["chunks"]) for ex in test_data) == 12_600
        )  # 12_601 reported in Shen et al. (2022), due to a duplicated entity
        ...
        assert (
            len(set([ex["org_id"] for ex in train_data + dev_data + test_data])) == 619
        )
    ...
    def test_nne_ringland2019acl(self):
        ...
        assert (
            sum(
                [
                    len(ex["chunks"]) - len(set((s, e) for _, s, e in ex["chunks"]))
    ...
    def test_conll2004(self):
        ...
        assert (
            max(
                detect_overlapping_level(ex["chunks"])
                for ex in train_data + dev_data + test_data
            )
            == FLAT
        )
        assert all(
            filter_clashed_by_priority(ex["chunks"], allow_level=FLAT) == ex["chunks"]
            for ex in train_data + dev_data + test_data
        )
    ...
    def test_SciERC(self):
        ...
        assert (
            sum(len(ex["relations"]) for ex in train_data) == 3_215
        )  # 4 duplicated relations dropped here
        ...
        assert all(
            filter_clashed_by_priority(ex["chunks"], allow_level=NESTED) == ex["chunks"]
            for ex in train_data + dev_data + test_data
        )
    ...
    def test_yidu_s4k(self):
        ...
        train_data1, train_errors1, train_mismatches1 = io.read(
            "data/yidu_s4k/subtask1_training_part1.txt", return_errors=True
        )
        train_data2, train_errors2, train_mismatches2 = io.read(
            "data/yidu_s4k/subtask1_training_part2.txt", return_errors=True
        )
        train_data, train_errors, train_mismatches = (
            train_data1 + train_data2,
            train_errors1 + train_errors2,
            train_mismatches1 + train_mismatches2,
        )
        test_data, test_errors, test_mismatches = io.read(
            "data/yidu_s4k/subtask1_test_set_with_answer.json", return_errors=True
        )
        ...
        assert (
            max(detect_overlapping_level(ex["chunks"]) for ex in train_data + test_data)
            == FLAT
        )
        assert all(
            filter_clashed_by_priority(ex["chunks"], allow_level=FLAT) == ex["chunks"]
    ...
    def test_cmeee(self):
        ...
        train_data, train_errors, train_mismatches = io.read(
            "data/cblue/CMeEE/CMeEE_train_vz.json", return_errors=True
        )
        dev_data, dev_errors, dev_mismatches = io.read(
            "data/cblue/CMeEE/CMeEE_dev_vz.json", return_errors=True
        )
        ...
        assert (
            max(detect_overlapping_level(ex["chunks"]) for ex in train_data + dev_data)
            == ARBITRARY
        )
        assert all(
            filter_clashed_by_priority(ex["chunks"], allow_level=ARBITRARY)
            == ex["chunks"]
            for ex in train_data + dev_data
        )
...
class TestSQuADIO(object):
    def test_squad_v2(self, spacy_nlp_en):
        ...
        train_data, train_errors, train_mismatches = io.read(
            "data/SQuAD/train-v2.0.json", return_errors=True
        )
        dev_data, dev_errors, dev_mismatches = io.read(
            "data/SQuAD/dev-v2.0.json", return_errors=True
        )
