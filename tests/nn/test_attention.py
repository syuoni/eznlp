...
@pytest.mark.parametrize(
    "scoring", ["Dot", "Scaled_Dot", "Multiplicative", "Additive", "Biaffine"]
)
...
def test_sequence_attention(num_heads, scoring, nonlinearity):
    ...
    atten_values, atten_weight = SequenceAttention(
        HID_DIM, num_heads=num_heads, scoring=scoring, nonlinearity=nonlinearity
    )(x, mask, return_atten_weight=True)
    if num_heads > 1:
