...
def test_rnn_last(use_mask):
    ...
    rnn = torch.nn.LSTM(
        HID_DIM, HID_DIM, num_layers=2, bidirectional=True, batch_first=True
    )
    x = torch.randn(BATCH_SIZE, MAX_LEN, HID_DIM)
    ...
    if use_mask:
        ...
        packed_x = torch.nn.utils.rnn.pack_padded_sequence(
            x, lengths=seq_lens, batch_first=True, enforce_sorted=False
        )
        packed_y, (hidden, _) = rnn(packed_x)
        y, _ = torch.nn.utils.rnn.pad_packed_sequence(
            packed_y, batch_first=True, padding_value=0
        )
    else:
        mask = None
        y, (hidden, _) = rnn(x)
...
@pytest.mark.parametrize(
    "x, group_by",
    [
        (torch.randn(1, 10, 20), torch.tensor([[0, 0, 1, 1, 2, 3, 3, 3, 3, -1]])),
        (torch.randn(16, 10, 20), torch.randint(0, 8, size=(16, 10))),
    ],
)
@pytest.mark.parametrize(
    "agg_mode, f_agg",
