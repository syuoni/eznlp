...
class TestMKMMDLoss(object):
    ...
    def test_against_third_party(self, num_kernels, multiplier):
        ...
        benchmark_mmd = third_party.transferlearning.code.distance.MMD_loss(
            kernel_num=num_kernels, kernel_mul=multiplier
        )
        benchmark_res = benchmark_mmd(x1, x2)

        mmd = MultiKernelMaxMeanDiscrepancyLoss(
            num_kernels=num_kernels, multiplier=multiplier
        )
        mmd_res = mmd(x1, x2)
