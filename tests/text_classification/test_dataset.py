...
class TestTextClassificationDataset(object):
    ...
    def test_pre_truncation(self, bert_with_tokenizer):
        ...
        val_data = [curr_data for curr_data in val_data if len(curr_data['tokens']) >= 500]
        assert max(len(curr_data['tokens']) for curr_data in val_data) > 510
        
        val_set = TextClassificationDataset(val_data, config)
        sub_lens = [sum(len(tokenizer.tokenize(word)) for word in curr_data['tokens'].raw_text) for curr_data in val_set.data]
