...
class OptionSampler(object):
    def __init__(self, **kwargs):
        for arg_name, arg_range in kwargs.items():
            if arg_range is None:
                ...
                assert all(
                    isinstance(arg_value, (bool, int, float, str))
    ...
    def evenly_sample(self, num_options):
        ...
        zip_options = [
            [
                self._parse_argument(arg_name, arg_value)
                for arg_value in self._evenly_sample_values(
                    arg_range, redundant_num_options
                )
            ]
    ...
    def fully_sample(self):
        option_space = [
            [self._parse_argument(arg_name, arg_value) for arg_value in arg_range]
            for arg_name, arg_range in self.__dict__.items()
        ]
    ...
    def sample(self, num_options=None):
        if num_options is None or num_options >= self.num_possible_options:
            logger.info(
                f"Sampling fully {self.num_possible_options}/{self.num_possible_options} options..."
            )
            ...
            logger.info(
                f"Sampling randomly {num_options}/{self.num_possible_options} options..."
            )
            ...
            logger.info(
                f"Sampling evenly {num_options}/{self.num_possible_options} options..."
            )
