...
class SpanBertLikeEncoder(torch.nn.Module):
    def __init__(self, config: SpanBertLikeConfig):
        ...
        if config.init_agg_mode.lower().endswith("_pooling"):
            ...
            for k in range(config.min_span_size, config.max_span_size + 1):
                ...
                conv = torch.nn.Conv1d(
                    config.hid_dim, config.hid_dim, kernel_size=k, groups=config.hid_dim
                )
        ...
        if config.share_weights_int:
            ...
            self.query_bert_like = torch.nn.ModuleList(
                [
    ...
    def forward(self, all_hidden_states: List[torch.Tensor]):
        ...
        for k in range(self.min_span_size, min(self.max_span_size, num_steps) + 1):
            ...
            reshaped_states = [
                hidden_states.unfold(dimension=1, size=k, step=1)
                .permute(0, 1, 3, 2)
                .flatten(end_dim=1)
            ...
            if isinstance(self.init_aggregating, (SequencePooling, SequenceAttention)):
                ...
                query_states = self.init_aggregating(
                    self.dropout(reshaped_states[0])
                ).unsqueeze(1)
            else:
                query_states = self.init_aggregating[k - self.min_span_size](
                    self.dropout(all_hidden_states[0].permute(0, 2, 1))
                ).permute(0, 2, 1)
            ...
            if hasattr(self, "size_embedding"):
                ...
                query_states = query_states + self.dropout(
                    size_embedded.expand_as(query_states)
                )

            if self.share_weights_int:
                query_outs = self.query_bert_like(query_states, reshaped_states)
            else:
                query_outs = self.query_bert_like[k - self.min_span_size](
                    query_states, reshaped_states
                )

            # query_states: (B, L-K+1, H)
            all_last_query_states[k] = query_outs["last_query_state"].view(
