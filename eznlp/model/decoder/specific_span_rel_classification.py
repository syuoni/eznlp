...
class SpecificSpanRelClsDecoder(DecoderBase, ChunkPairsDecoderMixin):
    ...
    def _collect_specific_contexts(
        self, all_hidden: List[torch.Tensor], i: int, cp_obj: ChunkPairs
    ):
        ...
        for (
            ...
            if not is_valid:
                ...
                if c_end - c_start <= max_span_size:
                    ...
                    curr_context = (
                        all_hidden[max_span_size - 1][i, c_start]
                        + all_hidden[max_span_size - 1][i, c_end - max_span_size]
                    ) / 2
    ...
    def get_logits(
        ...
        for i, cp_obj in enumerate(batch.cp_objs):
            if not cp_obj.has_valid_cp:
                ...
                span_hidden = torch.stack(
                    [
                        all_hidden[end - start - 1][i, start]
                        for label, start, end in cp_obj.chunks
                    ]
                )
                ...
                if self.fusing_mode.startswith("concat"):
                    ...
                    if self.use_context:
                        if self.context_mode.lower().count("shallow"):
                            ...
                            contexts = self._collect_specific_contexts(
                                all_hidden, i, cp_obj
                            )
