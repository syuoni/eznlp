...
class JointExtractionDecoderMixin(DecoderMixinBase):
    ...
    def evaluate(self, y_gold: List[List[tuple]], y_pred: List[List[tuple]]):
        return tuple(
            decoder.evaluate(set_tuples_gold, set_tuples_pred)
            for decoder, set_tuples_gold, set_tuples_pred in zip(
                self.decoders, y_gold, y_pred
            )
        )
...
class JointExtractionDecoderConfig(Config, JointExtractionDecoderMixin):
    ...
    def valid(self):
        return (
            all(decoder.valid for decoder in self.decoders)
            and len(list(self.decoders)) >= 2
        )
