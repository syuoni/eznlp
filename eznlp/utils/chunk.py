...
def filter_clashed_by_priority(chunks: List[tuple], allow_level: int = NESTED):
    ...
    for ck in chunks:
        if all(
            not _is_clashed(ck, ex_ck, allow_level=allow_level)
...
def detect_nested(
    chunks1: List[tuple], chunks2: List[tuple] = None, strict: bool = True
):
    ...
    for ck1 in chunks1:
        if any(
            _is_ordered_nested(ck1, ck2)
            and (ck1 != ck2)
            and (not strict or ck1[1:] != ck2[1:])
            for ck2 in chunks2
        ):
...
class TextChunksTranslator(object):
    ...
    def text_chunks2chunks(
        self,
        text_chunks: List[tuple],
        tokens: TokenSequence,
        raw_text: str = None,
        place_none_for_errors: bool = False,
    ):
        ...
        for (
            ...
            *possible_chunk_text,
            ...
            if len(possible_chunk_text) > 0:
                ...
                # Error data
                if not self.is_consistency(
                    chunk_text, raw_text[chunk_start_in_text:chunk_end_in_text]
                ):
                    if self.is_consistency(
                        chunk_text,
                        raw_text[
                            chunk_start_in_text : chunk_start_in_text + len(chunk_text)
                        ],
                    ):
                        chunk_end_in_text = chunk_start_in_text + len(chunk_text)
                    elif self.is_consistency(
                        chunk_text,
                        raw_text[
                            chunk_end_in_text - len(chunk_text) : chunk_end_in_text
                        ],
                    ):
                        ...
                        errors.append(
                            (
                                chunk_text,
                                raw_text[chunk_start_in_text:chunk_end_in_text],
                            )
                        )
            ...
            if not find_start:
                mismatches.append(
                    (
                        raw_text[
                            text_starts[chunk_start - 1] : text_ends[chunk_start - 1]
                        ],
            ...
            if not find_end:
                mismatches.append(
                    (
                        raw_text[text_starts[chunk_end_m1] : text_ends[chunk_end_m1]],
            ...
            # If it is almost found...
                ...
                and find_end
                and (
                    chunk_start_in_text - text_starts[chunk_start - 1]
                    <= self.mismatch_tol
                )
            ):
                ...
                find_start
                and (not find_end)
                and (text_ends[chunk_end_m1] - chunk_end_in_text <= self.mismatch_tol)
            ):
                chunks.append((chunk_type, chunk_start, chunk_end_m1 + 1))
    ...
    def chunks2text_chunks(
        self,
        chunks: List[tuple],
        tokens: TokenSequence,
        raw_text: str = None,
        append_chunk_text: bool = False,
    ):
        text_starts, text_ends = tokens.start, tokens.end
