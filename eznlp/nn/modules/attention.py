    ...
    * If `external_query` is False, the query vector should be passed from outside.
    ...
    def __init__(
        ...
        if scoring.lower() in ("dot", "scaled_dot"):
            assert (
                query_dim == key_dim
            ), f"`query_dim` {query_dim} does not equals `key_dim` {key_dim}"

        elif scoring.lower() == "multiplicative":
            self.proj_layer = torch.nn.Linear(
                key_dim // num_heads, query_dim // num_heads
            )
            reinit_layer_(self.proj_layer, nonlinearity)

        elif scoring.lower() == "additive":
            self.proj_layer = torch.nn.Linear(
                (key_dim + query_dim) // num_heads, atten_dim
            )
            reinit_layer_(self.proj_layer, nonlinearity)
    ...
    def compute_scores(self, query: torch.Tensor, key: torch.Tensor):
        ...
        if self.scoring.lower() == "dot":
            ...
            key_query = torch.cat(
                [
                    key.unsqueeze(1).expand(-1, query.size(1), -1, -1),
                    query.unsqueeze(2).expand(-1, -1, key.size(1), -1),
                ],
                dim=-1,
            )
            ...
            key_query = self.key_proj_layer(key).unsqueeze(1) + self.query_proj_layer(
                query
            ).unsqueeze(2)
    ...
    def _prepare_multiheads(self, x: torch.Tensor):
        ...
        return (
            x.view(batch_size, -1, self.num_heads, dim_per_head)
            .permute(0, 2, 1, 3)
            .contiguous()
            .view(batch_size * self.num_heads, -1, dim_per_head)
        )
    ...
    def _restore_multiheads(self, x: torch.Tensor):
        ...
        return (
            x.view(batch_size, self.num_heads, -1, dim_per_head)
            .permute(0, 2, 1, 3)
            .contiguous()
            .view(batch_size, -1, dim_per_head * self.num_heads)
        )
    ...
    def forward(
        ...
        if mask is None:
            ...
            atten_weight = torch.nn.functional.softmax(
                scores.masked_fill(mask, float("-inf")), dim=-1
            )
            atten_weight = self.dropout(
                atten_weight
            )  # Apply dropout on attention weight?
        ...
        if self.num_heads > 1:
            ...
            atten_weight = atten_weight.view(
                atten_values.size(0),
                self.num_heads,
                atten_weight.size(-2),
                atten_weight.size(-1),
...
class MultiheadAttention(torch.nn.Module):
    ...
    def forward(
        ...
        atten_values, atten_weight = self.attention(
            VW, mask=mask, query=QW, key=KW, return_atten_weight=True
        )
