...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_data.add_argument(
        "--dataset", type=str, default="conll2004", help="dataset name"
    )
    group_data.add_argument(
        "--doc_level",
        default=False,
    ...
    group_decoder.add_argument(
        "--rel_decoder",
        type=str,
        default="span_rel_classification",
        help="relation decoding method",
        choices=[
            "span_rel_classification",
            "specific_span_rel",
            "unfiltered_specific_span_rel",
            "masked_span_rel",
        ],
    )
    ...
    group_decoder.add_argument(
        "--context_mode", type=str, default="specific", help="context mode"
    )
    group_decoder.add_argument(
        "--context_ext_win", type=int, default=0, help="window size of extended context"
    )
    group_decoder.add_argument(
        "--context_no_exc_ck",
        dest="context_exc_ck",
...
def build_RE_config(args: argparse.Namespace):
    ...
    if args.rel_decoder == "span_rel_classification":
        decoder_config = SpanRelClassificationDecoderConfig(
        ...
        decoder_config = SpecificSpanRelClsDecoderConfig(
            use_context=args.use_context,
        ...
        decoder_config = UnfilteredSpecificSpanRelClsDecoderConfig(
            fusing_mode=args.fusing_mode,
            reduction=reduction_config,
            fl_gamma=args.fl_gamma,
            sl_epsilon=args.sl_epsilon,
...
if __name__ == "__main__":
    ...
    if args.dataset.startswith("ace2004_rel_cv") or args.dataset in (
        "ace2005_rel",
        "SciERC",
    ):
        ...
        for data in [train_data, dev_data, test_data]:
            counter = Counter()
            for entry in data:
                missing_relations = detect_missing_symmetric(
                    entry["relations"], config.decoder.sym_rel_labels
                )
                counter.update([rel[0] for rel in missing_relations])
                entry["relations"].extend(missing_relations)
            logger.info(
                "Adding missing symmetric relations: "
                + " | ".join(f"{label}: {c}" for label, c in counter.items())
            )
