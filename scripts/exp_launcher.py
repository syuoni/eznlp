...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--task", type=str, default="entity_recognition", help="task name"
    )
    ...
    parser.add_argument(
        "--num_exps", type=int, default=-1, help="number of experiments to run"
    )
    parser.add_argument(
        "--num_workers", type=int, default=0, help="number of processes to run"
    )
    ...
    COMMAND = (
        f"python scripts/{args.task}.py --dataset {args.dataset} --seed {args.seed}"
    )
    ...
    if args.task == "text_classification":
        ...
        if not args.use_bert:
            ...
            sampler = OptionSampler(
                ...
                finetune_lr=numpy.logspace(
                    -4.5, -4.0, num=100, base=10
                ).tolist(),  # 3e-5 ~ 1e-4
                batch_size=48,
                ck_decoder="span_classification",
                bert_drop_rate=0.2,
