...
def parse_arguments(parser: argparse.ArgumentParser):
    ...
    group_decoder.add_argument(
        ...
        choices=["None", "span_attr_classification"],
    )
    group_decoder.add_argument(
        "--rel_decoder",
        type=str,
        ...
        choices=["None", "span_rel_classification"],
    )
    ...
    group_decoder.add_argument(
        "--max_size_id", type=int, default=9, help="maximum span size ID"
    )
    group_decoder.add_argument(
        "--size_emb_dim", type=int, default=25, help="span size embedding dim"
    )
    group_decoder.add_argument(
        "--label_emb_dim", type=int, default=25, help="chunk label embedding dim"
    )
...
def build_joint_config(args: argparse.Namespace):
    ...
    if args.ck_decoder == "sequence_tagging":
        ...
        ck_decoder_config = SpanClassificationDecoderConfig(
    ...
    if args.attr_decoder == "None":
        ...
        attr_decoder_config = SpanAttrClassificationDecoderConfig(
            agg_mode=args.agg_mode,
            neg_sampling_rate=args.neg_sampling_rate,
    ...
    if args.rel_decoder == "None":
        ...
        rel_decoder_config = SpanRelClassificationDecoderConfig(
            agg_mode=args.agg_mode,
            fl_gamma=args.fl_gamma,
            sl_epsilon=args.sl_epsilon,
            neg_sampling_rate=args.neg_sampling_rate,
    ...
    decoder_config = JointExtractionDecoderConfig(
...
if __name__ == "__main__":
    ...
    evaluate_joint_extraction(
        trainer,
        dev_set,
        has_attr=(args.attr_decoder != "None"),
        has_rel=(args.rel_decoder != "None"),
        batch_size=args.batch_size,
    )
