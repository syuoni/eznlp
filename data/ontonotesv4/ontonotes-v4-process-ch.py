...
def parse_sentence(sent):
    ...
    for ent in re.finditer('<ENAMEX TYPE="\w+".*?>.+?</ENAMEX>', sent):
        ...
        entities.append(
            (ent_type, len(tokenized), len(tokenized) + len(ent_text_tokenized))
        )
...
files = glob.glob(
    "../ontonotes-release-4.0/data/files/data/chinese/annotations/*/*/*/*.name"
)
print(len(files))
...
dev_files = [
    fn
    for fn in files
    if os.path.basename(fn).startswith("chtb")
    and int(re.search("\d+", os.path.basename(fn)).group()) % 2 == 1
]
test_files = [
    fn
    for fn in files
    if os.path.basename(fn).startswith("chtb")
    and int(re.search("\d+", os.path.basename(fn)).group()) % 2 == 0
]
train_files = [fn for fn in files if fn not in dev_files + test_files]


for split_name, split_files in zip(
    ["dev", "test", "train"], [dev_files, test_files, train_files]
):
